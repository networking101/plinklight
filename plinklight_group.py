...
def main():
    usage = "Linux group query over ssh"
    ...
    Enum_group(targetFile, username, password, csvpath)
...
def Enum_group(targetFile, username, password, csvpath):

    # create final csv with compiled results
    header_saved = False
    header = 'GROUP,PASSWORD,GID,GROUP LIST,HOST\n'
    masterfile = csvpath + '/group_' + dateName + '.csv'
	...
	# run command and save to file
                print "running ps"
		command = 'sudo cat /etc/group | tr \',\' \';\' | tr \':\' \',\' | awk \'{print $0",' + targ.rstrip() + '"}\' > kapow.txt'
                #print p.before
                p.sendline(command)
		p.delaybeforesend = 1
		if username != "root":
	...
	# Copy file back to MIP
                print '\nCopying file to MIP\n'
                homefile = csvpath + '/group_' + targ.rstrip() + '.csv'
