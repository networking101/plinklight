...
def main():
    usage = "Linux process query over ssh"
...
def Enum_proc(targetFile, username, password, csvpath):
    ...
    header_saved = False
    header = '%CPU,GROUP,PPID,USER,COMMAND,PROCESS,RGROUP,NI,PID,PGID,ELAPSED,RUSER,TIME,TTY,VSZ,HOST\n'
    masterfile = csvpath + '/enum_proc_' + dateName + '.csv'
	...
	# run command and save to file
                print "running ps\n"
		command = 'sudo ps -Ao "%C,%G,%P,%U,%a,%c,%g,%n,%p,%r,%t,%u,%x,%y,%z" | awk \'{print $0",' + targ.rstrip() + '"}\' | grep -v %CPU | grep -v -i  "ps -Ao" | grep -v -i "awk {print \$0" > kapow.txt'
                #print p.before
                p.sendline(command)
		p.delaybeforesend = 1
		if username != "root":
	...
	# Copy file back to MIP
                print '\nCopying file to MIP\n'
                homefile = csvpath + '/enum_proc_' + targ.rstrip() + '.csv'
