...
def main():
    usage = "Linux network query over ssh"
    ...
    Enum_net(targetFile, username, password, csvpath)
...
def Enum_net(targetFile, username, password, csvpath):

    # create final csv with compiled results
    header_saved = False
    header = 'PROTOCOL,RECV-Q,SEND-Q,LOCAL ADDRESS,LOCAL PORT,FOREIGN ADDRESS,FOREIGN PORT,HOST,PID,PROGRAM,STATE\n'
    masterfile = csvpath + '/enum_net_' + dateName + '.csv'
	...
	# run command and save to file
                print "running ps\n"
                command = 'sudo netstat -plantu | grep -v \"Active Internet connections\" | grep -v Proto | sed \'s/: /-/g\' | sed \'s/:://g\' | sed \'s/ \[/\[/g\' | awk \'{print $0"' + targ.rstrip() + '"}\' | awk \' { t=$6; $6=$8; $8=t; print; }\' | grep -v \'TIME_WAIT\'| tr -s \' \' | tr \' \' \',\' | tr \':\' \',\' | tr \'/\' \',\' > kapow.txt'
		#print p.before
                p.sendline(command)
		p.delaybeforesend = 1
		if username != "root":
	...
	# Copy file back to MIP
                print '\nCopying file to MIP\n'
                homefile = csvpath + '/enum_net_' + targ.rstrip() + '.csv'
