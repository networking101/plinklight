...
def main():
    usage = "Linux query logged on users over ssh"
    ...
    Enum_logon(targetFile, username, password, csvpath)
...
def Enum_logon(targetFile, username, password, csvpath):

    # create final csv with compiled results
    header_saved = False
    header = 'USER,PSEUDO TERMINAL,SOURCE,DAY,TIME STAMP,STATUS,HOST\n'
    masterfile = csvpath + '/logon_' + dateName + '.csv'
	...
	# run command and save to file
                print "running ps\n"
		command = 'sudo last -f /var/run/utmp | grep -v reboot | awk \'{print $0"' + targ.rstrip() + '"}\' | grep \'\\n\'| grep -v "utmp begins" | tr -s \' \' | tr \' \' \',\' | sed \'s/\(\([^,]*,\)\{4\}[^,]*\),/\\1-/\' | sed \'s/\(\([^,]*,\)\{4\}[^,]*\),/\\1-/\' | sed \'s/still,logged,in/still logged in/g\' > kapow.txt'
		#print p.before
                p.sendline(command)
		p.delaybeforesend = 1
		if username != "root":
	...
	# Copy file back to MIP
                print '\nCopying file to MIP\n'
                homefile = csvpath + '/logon_' + targ.rstrip() + '.csv'
