...
def Enum_user(targetFile, username, password, csvpath):
    ...
    header_saved = False
    header = 'USER NAME,PASSWORD,UID,GID,FULL NAME,HOME DIR,SHELL,HOST\n'
	...
	# run command and save to file
                print "running ps"
		command = 'sudo cat /etc/passwd | tr \':\' \',\' | awk \'{print $0",' + targ.rstrip() + '"}\' > kapow.txt'
                #print p.before
                p.sendline(command)
		p.delaybeforesend = 1
		if username != "root":
	...
	# Copy file back to MIP
                print '\nCopying file to MIP\n'
                homefile = csvpath + '/users_' + targ.rstrip() + '.csv'
