...
def main():
    usage = "Linux shadow query over ssh"
    ...
    Enum_shadow(targetFile, username, password, csvpath)
...
def Enum_shadow(targetFile, username, password, csvpath):

    # create final csv with compiled results
    header_saved = False
    header = 'USER NAME,PASSWORD,LAST CHANGED,MIN PASS CHANGE,MAX PASS CHANGE,WARN BEFORE PASS CHANGE,INACTIVE,ACCOUNT EXPIRE,TIME DISABLED,HOST\n'
    masterfile = csvpath + '/shadow_' + dateName + '.csv'
	...
	# run command and save to file
                print "running cat /etc/shadow\n"
		command = 'sudo cat /etc/shadow | tr \':\' \',\' | awk \'{print $0",' + targ.rstrip() + '"}\' > kapow.txt'
                #print p.before
                p.sendline(command)
		p.delaybeforesend = 1
		if username != "root":
	...
	# Copy file back to MIP
                print '\nCopying file to MIP\n'
                homefile = csvpath + '/shadow_' + targ.rstrip() + '.csv'
