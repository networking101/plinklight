...
def main():
    usage = "Linux scheduled tasks query over ssh"
    ...
    Enum_cron(targetFile, username, password, csvpath)
...
def Enum_cron(targetFile, username, password, csvpath):

    # create final csv with compiled results
    header_saved = False
    header = 'PROPERTIES,HARD LINKS,OWNER,GROUP,SIZE (B),TIME STAMP,FILE,HOST,INTERVAL\n'
    masterfile = csvpath + '/cron_' + dateName + '.csv'
	...
	# run command and save to file
                print "running ps\n"
		command = 'ls -al /etc/cron.hourly | awk \'{print $0",' + targ.rstrip() + '"}\' | awk \'{print $0",hourly"}\' | tr -s \' \' | tr \' \' \',\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | grep -v \',\.\.,\' | grep -v \',\.,\' | grep -v \'total\' | cut -c 2- > kapow.txt; ls -al /etc/cron.daily | awk \'{print $0",' + targ.rstrip() + '"}\' | awk \'{print $0",daily"}\' | tr -s \' \' | tr \' \' \',\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | grep -v \',\.\.,\' | grep -v \',\.,\' | grep -v \'total\' | cut -c 2- >> kapow.txt; ls -al /etc/cron.weekly | awk \'{print $0",' + targ.rstrip() + '"}\' | awk \'{print $0",weekly"}\' | tr -s \' \' | tr \' \' \',\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | grep -v \',\.\.,\' | grep -v \',\.,\' | grep -v \'total\' | cut -c 2- >> kapow.txt; ls -al /etc/cron.monthly | awk \'{print $0",' + targ.rstrip() + '"}\' | awk \'{print $0",monthly"}\' | tr -s \' \' | tr \' \' \',\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | grep -v \',\.\.,\' | grep -v \',\.,\' | grep -v \'total\' | cut -c 2- >> kapow.txt;ls -al /etc/cron.d | awk \'{print $0",' + targ.rstrip() + '"}\' | awk \'{print $0",d"}\' | tr -s \' \' | tr \' \' \',\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | sed \'s/\(\([^,]*,\)\{5\}[^,]*\),/\\1-/g\' | grep -v \',\.\.,\' | grep -v \',\.,\' | grep -v \'total\' | cut -c 2- >> kapow.txt'
                #print p.before
                p.sendline(command)
		p.delaybeforesend = 1
	...
	# Copy file back to MIP
                print '\nCopying file to MIP\n'
                homefile = csvpath + '/cron_' + targ.rstrip() + '.csv'
